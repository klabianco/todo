...
export const createTaskElement = (
    ...
    if (showTimes) {
        ...
        if (hasTime) {
            ...
            // Determine time of day icon (using Font Awesome)
            if (h >= 5 && h < 12) {
                // Morning (5am - 11:59am) - sunrise/cloud-sun icon (orange)
                timeOfDay = 'morning';
                timeIcon = `<i class="fa-solid fa-cloud-sun text-orange-400"></i>`;
            } else if (h >= 12 && h < 18) {
                // Afternoon (12pm - 5:59pm) - bright sun icon (yellow)
                timeOfDay = 'afternoon';
                timeIcon = `<i class="fa-solid fa-sun text-yellow-500"></i>`;
            } else {
                // Evening/Night (6pm - 4:59am) - moon icon (indigo)
                timeOfDay = 'evening';
                timeIcon = `<i class="fa-solid fa-moon text-indigo-400"></i>`;
            }
        }
